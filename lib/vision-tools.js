...
import { sessionHeaders } from './session-header.js'
...
export const CONTENT_FILTER_RE =
  /敏感|不安全|内容审核|内容安全|政策|content.?filter|safety|inappropriate|flagged|拒绝.?内容|违反.*(规范|政策|安全)|violat/i
...
export async function callVisionModel(getConfig, prompt, imageUrls, options = {}) {
  ...
  const sessionHeader = sessionHeaders(cfg, options.exec)
...
export function registerVisionTools(ctx, getConfig) {
  ...
  // ── vision_screenshot (privacy-gated: only registered when desktopScreenshot is enabled) ──
  if (getConfig().desktopScreenshot === true) {
    tools.push({
