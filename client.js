...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      hintAutoDetectMultimodal: "默认开启：按路由自己声明的 inputModalities 判定，识别到纯文本就交给桥接、识别到多模态就等同白名单成员直发图片；声明永远读“包装前”的真值，不会被插件的自动声明污染。声明可能不实——命中后若上游拒收图片，把该模型写进下面的名单并切到黑名单模式即可强制桥接（首次放行时日志里也会提示）。",
      catalogTitle: "当前路由判定",
    ...
    var en = {
      ...
      hintAutoDetectMultimodal: "On by default: the route's own declared inputModalities decides — a text-only route is bridged, a multimodal one is treated like a whitelist member and gets images directly. The declaration is read BEFORE the plugin's admission wrap, so its own claim can never become the evidence. A declaration can lie: if the endpoint rejects images, list that model below and switch to blacklist mode to force the bridge (the first promotion also logs a notice).",
